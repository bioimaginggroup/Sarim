  ...
  */
  ...
  for (int n_mcmc = 1; n_mcmc <= (nIter+burnin); ++n_mcmc) {
    ...
    for (int k = 0; k < p; ++k) {
      ...
      // compute p(ga_c | \ka^c)
      prior_c = - (ka_vector.row(0) * 0.5) * gamma_current.transpose() * K_k * gamma_current;
      
      // compute p(ga_p | \ka^c)
      prior_p = - (ka_vector.row(0) * 0.5) * gamma_proposal.transpose() * K_k * gamma_proposal;
      ...
      ka_vector.row(0) = random_gamma(1, ka_alpha, 1/ka_beta);
