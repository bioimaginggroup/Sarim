...
time.taken
# Time difference of 1.022992 hours

# save results for later inspection
setwd("~/Schreibtisch/Master/C++/Abgabe/simulation_gaussian")
saveRDS(mod_gaussian, "simulation_gaussian_output.rds")
#mod_gaussian <- readRDS("simulation_gaussian_output.rds")

################################################################################
# take a look at the results
mod_gaussian$lanzcos_iterations


ga1 <- apply(mod_gaussian$coef_results[[1]][, 500:3501], 1, FUN = mean)
...
ga2 <- apply(mod_gaussian$coef_results[[2]][, 500:3501], 1, FUN = mean)
...
ga3 <- apply(mod_gaussian$coef_results[[3]][, 500:3501], 1, FUN = mean)
...
hist(mod_gaussian$coef_results[[4]][500:3501], breaks = 50)
ga4 <- mean(mod_gaussian$coef_results[[4]][500:3501])
ga4


hist(mod_gaussian$coef_results[[5]][500:3501], breaks = 50)
ga5 <- mean(mod_gaussian$coef_results[[5]][500:3501])
ga5


hist(mod_gaussian$sigma_results[500:3501], breaks = 50)
